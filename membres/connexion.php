			...
			<div id="contenu">
				...
				<form name="connexion" id="connexion" method="post" action="connexion.php">
					<fieldset><legend>Connexion</legend>
						<label for="speudo" class="float">Pseudo :</label> <input type="text" name="speudo" id="speudo" value="<?php //if(isset($_SESSION['connexion_pseudo'])) echo $_SESSION['connexion_pseudo']; ?>"/><br/>
						<label for="password" class="float">Passe :</label> <input type="password" name="password" id="password"/><br/>
				...
				{
					$result = sqlquery("SELECT COUNT(id_pseudo) AS nbr, id_pseudo, pseudo, password FROM connection WHERE
					pseudo = '".mysql_real_escape_string($_POST['pseudo'])."' GROUP BY id_pseudo", 1);
					
					if($result['nbr'] == 1)
					{
						if(md5($_POST['password']) == $result['password'])
						{
							$_SESSION['id_pseudo'] = $result['id_pseudo'];
							...
							{
								setcookie('id_pseudo', $result['id_pseudo'], time()+365*24*3600);
							...
							$informations = Array(/*Vous êtes bien connecté*/
											false,
											'Connexion réussie',
											'Vous êtes désormais connecté avec le pseudo <span class="pseudo">'.htmlspecialchars($_SESSION['pseudo'], ENT_QUOTES).'</span>.',
											'',
											ROOTPATH.'/index.php',
											3
											);
